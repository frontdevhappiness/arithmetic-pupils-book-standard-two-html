(function () {
  ...
  function buildPage37ExampleMap(_content, source, narration) {
    ...
    var mapping = new Array(narration.length).fill(null);
    // Each column describes its coloured counters, addition, and completed total.
    columns.forEach(function (column, columnIndex) {
      var header = column.querySelector("h3");
      var counters = column.querySelectorAll(".counter-row");
      var words = column.querySelectorAll(".model-word");
      var offset = columnIndex * 20;
      function assign(start, end, target) {
        for (var index = start; index <= end; index += 1) mapping[offset + index] = target;
      }
      assign(0, 3, header);
      assign(4, 6, counters[0]);
      assign(7, 9, words[0]);
      assign(10, 12, counters[1]);
      assign(13, 13, words[1]);
      assign(14, 16, counters[2]);
      assign(18, 18, words[2]);
      assign(19, 19, header);
    });
    return mapping;
  }
  ...
  function buildPage102Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg102_p001" && id !== "pg102_p017") return null;
    var section = content.querySelector('[data-section-id="pg102_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg102_p001") {
      ...
      [exercise && exercise.querySelector(".label"), exercise && exercise.querySelector(".prompt:not(.story)"), exercise && exercise.querySelector(".story")].forEach(function (root) {
        exerciseMap.push.apply(exerciseMap, ranges(root));
      });
      exercise && exercise.querySelectorAll(".question").forEach(function (question) {
        exerciseMap.push.apply(exerciseMap, ranges(question.querySelector(".qno")));
        exerciseMap.push.apply(exerciseMap, ranges(question.querySelector(".question-text")));
      });
      return exerciseMap.length === 113 && exerciseMap.every(Boolean) ? exerciseMap : [];
    }

    var ictMap = [];
    ictMap.push.apply(ictMap, ranges(section.querySelector(".ict-title")));
    ictMap.push.apply(ictMap, ranges(section.querySelector(".ict-intro")));
    var example = section.querySelector(".example");
    ictMap.push.apply(ictMap, ranges(example && example.querySelector(".label")));
    var equation = ranges(example && example.querySelector(".equation"));
    if (equation.length !== 4) return [];
    ictMap.push(equation[0], equation[1], equation[1], equation[2], equation[3]);
    ictMap.push.apply(ictMap, ranges(example && example.querySelector(".solution")));
    ictMap.push.apply(ictMap, ranges(example && example.querySelector(".method")));
    return ictMap.length === 41 && ictMap.every(Boolean) ? ictMap : [];
  }

  function buildPage103Map(content, source) {
    if (source.getAttribute("data-id") !== "pg103_p001") return null;
    var section = content.querySelector('[data-section-id="pg103_sec001"]');
    if (!section) return [];
    ...
    var screens = section.querySelector(".pg103-screens");
    var images = section.querySelectorAll(".pg103-screen");
    if (!screens || images.length !== 2) return [];
    var mapping = [];
    for (var overviewIndex = 0; overviewIndex < 6; overviewIndex += 1) mapping.push(screens);
    for (var questionIndex = 0; questionIndex < 4; questionIndex += 1) mapping.push(images[0]);
    for (var answerIndex = 0; answerIndex < 10; answerIndex += 1) mapping.push(images[1]);
    append(mapping, section.querySelector(".pg103-heading"));
    section.querySelectorAll(".pg103-step").forEach(function (step) {
      append(mapping, step.querySelector(".pg103-number"));
      append(mapping, step.querySelector(".pg103-text"));
    });
    return mapping.length === 64 && mapping.every(Boolean) ? mapping : [];
  }

  function buildPage104Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg104_p001" && id !== "pg104_p006") return null;
    var section = content.querySelector('[data-section-id="pg104_sec001"]');
    if (!section) return [];

    function ranges(root) {
      return root ? collectVisibleTokens(root).map(function (token) { return token.range; }) : [];
    }

    if (id === "pg104_p001") {
      var introduction = [];
      introduction.push.apply(introduction, ranges(section.querySelector(".chapter-label")));
      introduction.push.apply(introduction, ranges(section.querySelector(".chapter-title")));
      introduction.push.apply(introduction, ranges(section.querySelector(".topic")));
      var introWords = ranges(section.querySelector(".intro"));
      if (introWords.length !== 47) return [];
      introduction.push.apply(introduction, introWords.slice(0, 34));
      introduction.push([introWords[34], introWords[35]]);
      introduction.push.apply(introduction, introWords.slice(36));
      return introduction.length === 56 && introduction.every(Boolean) ? introduction : [];
    }

    var mapping = [];
    section.querySelectorAll(".example").forEach(function (example) {
      var label = ranges(example.querySelector(".example-label"));
      var table = example.querySelector(".fraction-table");
      var headers = table && table.querySelectorAll("th");
      var pictures = table && table.querySelectorAll("td.picture");
      var captions = table && table.querySelectorAll("td.caption");
      if (label.length !== 2 || !table || !headers || headers.length !== 3 || !pictures || pictures.length !== 3 || !captions || captions.length !== 3) return;
      mapping.push.apply(mapping, label);
      for (var summaryIndex = 0; summaryIndex < 5; summaryIndex += 1) mapping.push(table);
      mapping.push.apply(mapping, ranges(headers[0]));
      mapping.push.apply(mapping, ranges(headers[1]));
      mapping.push(table);
      mapping.push.apply(mapping, ranges(headers[2]));
      [2, 3, 3].forEach(function (captionWords, column) {
        for (var pictureWord = 0; pictureWord < 4; pictureWord += 1) mapping.push(pictures[column]);
        var caption = ranges(captions[column]);
        if (caption.length === captionWords) mapping.push.apply(mapping, caption);
      });
    });
    return mapping.length === 68 && mapping.every(Boolean) ? mapping : [];
  }
  ...
  function buildPage105Map(content, source) {
    ...
    if (id === "pg105_p001") {
      var example = section.querySelector(".pg105-example");
      var table = example && example.querySelector(".pg105-object-table");
      var cells = table && table.children;
      if (!example || !table || !cells || cells.length !== 9) return [];
      var exampleMap = ranges(example.querySelector(".pg105-label"));
      repeat(exampleMap, table, 5);
      exampleMap.push.apply(exampleMap, ranges(cells[0]));
      exampleMap.push.apply(exampleMap, ranges(cells[1]));
      exampleMap.push(table);
      exampleMap.push.apply(exampleMap, ranges(cells[2]));
      [2, 3, 3].forEach(function (captionWords, column) {
        repeat(exampleMap, cells[3 + column], 4);
        var caption = ranges(cells[6 + column]);
        if (caption.length === captionWords) exampleMap.push.apply(exampleMap, caption);
      });
      exampleMap.push.apply(exampleMap, ranges(section.querySelector(".pg105-writing h2")));
      var writing = section.querySelector(".pg105-writing p");
      var writingWords = ranges(writing);
      var writingFraction = writing && writing.querySelector(".pg105-fraction");
      if (writingWords.length !== 19 || !writingFraction) return [];
      exampleMap.push.apply(exampleMap, writingWords.slice(0, 8));
      exampleMap.push(writingFraction);
      exampleMap.push.apply(exampleMap, writingWords.slice(8));
      return exampleMap.length === 57 && exampleMap.every(Boolean) ? exampleMap : [];
    }

    var exercise = section.querySelector(".pg105-exercise");
    var directions = exercise && exercise.querySelector(".pg105-directions");
    var directionWords = ranges(directions);
    var directionFraction = directions && directions.querySelector(".pg105-fraction");
    var grid = exercise && exercise.querySelector(".pg105-exercise-grid");
    var gridCells = grid && grid.children;
    if (!exercise || directionWords.length !== 15 || !directionFraction || !grid || !gridCells || gridCells.length !== 8) return [];
    var exerciseMap = ranges(exercise.querySelector(".pg105-label"));
    exerciseMap.push.apply(exerciseMap, directionWords.slice(0, 5));
    exerciseMap.push(directionFraction);
    exerciseMap.push.apply(exerciseMap, directionWords.slice(5));
    exerciseMap.push.apply(exerciseMap, ranges(gridCells[0]));
    repeat(exerciseMap, gridCells[1], 14);
    repeat(exerciseMap, gridCells[2], 4);
    exerciseMap.push.apply(exerciseMap, ranges(gridCells[2].querySelector(".pg105-answer-word")));
    exerciseMap.push(gridCells[3]);
    var answerFraction = ranges(gridCells[3]);
    if (answerFraction.length !== 2) return [];
    exerciseMap.push(answerFraction[0], gridCells[3], answerFraction[1]);
    exerciseMap.push.apply(exerciseMap, ranges(gridCells[4]));
    repeat(exerciseMap, gridCells[5], 14);
    exerciseMap.push(gridCells[6]);
    exerciseMap.push.apply(exerciseMap, ranges(gridCells[6].querySelector(".pg105-answer-word")));
    repeat(exerciseMap, gridCells[7], 8);
    return exerciseMap.length === 69 && exerciseMap.every(Boolean) ? exerciseMap : [];
  }

  function buildPage106Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg106_p001") return null;
    var section = content.querySelector('[data-section-id="pg106_sec001"]');
    if (!section) return [];
    var rows = section.querySelectorAll(".pg106-table > .pg106-cell");
    ...
    function mapNameAndNumeral(mapping, nameCell, numeralCell) {
      repeat(mapping, nameCell.querySelector(".pg106-blank"), 3);
      repeat(mapping, numeralCell, 6);
    }
    function mapNamedHalf(mapping, nameCell, numeralCell) {
      repeat(mapping, nameCell, 4);
      var name = ranges(nameCell.querySelector(".pg106-name"));
      if (name.length !== 2) return false;
      mapping.push.apply(mapping, name);
      repeat(mapping, numeralCell.querySelector(".pg106-blank"), 3);
      return true;
    }

    var mapping = [];
    repeat(mapping, section.querySelector(".pg106-table"), 3);

    mapping.push.apply(mapping, ranges(rows[0]));
    repeat(mapping, rows[1], 14);
    mapNameAndNumeral(mapping, rows[2], rows[3]);

    mapping.push.apply(mapping, ranges(rows[4]));
    repeat(mapping, rows[5], 15);
    if (!mapNamedHalf(mapping, rows[6], rows[7])) return [];

    mapping.push.apply(mapping, ranges(rows[8]));
    repeat(mapping, rows[9], 14);
    mapNameAndNumeral(mapping, rows[10], rows[11]);

    mapping.push.apply(mapping, ranges(rows[12]));
    repeat(mapping, rows[13], 9);
    if (!mapNamedHalf(mapping, rows[14], rows[15])) return [];

    mapping.push.apply(mapping, ranges(rows[16]));
    repeat(mapping, rows[17], 16);
    mapNameAndNumeral(mapping, rows[18], rows[19]);
    return mapping.length === 121 && mapping.every(Boolean) ? mapping : [];
  }

  function buildPage107Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg107_p001" && id !== "pg107_p013") return null;
    var section = content.querySelector('[data-section-id="pg107_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg107_p001") {
      var exercise = section.querySelector(".pg107-exercise");
      var items = exercise && Array.from(exercise.querySelectorAll(".pg107-item"));
      if (!exercise || !items || items.length !== 9) return [];
      items.sort(function (first, second) {
        return Number(first.querySelector(".pg107-qno").textContent) - Number(second.querySelector(".pg107-qno").textContent);
      });
      var descriptionLengths = [8, 15, 14, 14, 13, 10, 11, 16, 11];
      var exerciseMap = ranges(exercise.querySelector(".pg107-label"));
      exerciseMap.push.apply(exerciseMap, ranges(exercise.querySelector(".pg107-instruction")));
      items.forEach(function (item, itemIndex) {
        exerciseMap.push.apply(exerciseMap, ranges(item.querySelector(".pg107-qno")));
        repeat(exerciseMap, item.querySelector("img"), descriptionLengths[itemIndex]);
      });
      return exerciseMap.length === 142 && exerciseMap.every(Boolean) ? exerciseMap : [];
    }

    var copy = section.querySelector(".pg107-copy");
    var paragraphs = copy && copy.querySelectorAll("p");
    if (!copy || !paragraphs || paragraphs.length !== 2) return [];
    var copyMap = ranges(copy.querySelector(".pg107-heading"));
    copyMap.push.apply(copyMap, ranges(paragraphs[0]));
    var numeralWords = ranges(paragraphs[1]);
    var fraction = paragraphs[1].querySelector(".pg107-fraction");
    if (numeralWords.length !== 19 || !fraction) return [];
    copyMap.push.apply(copyMap, numeralWords.slice(0, 8));
    copyMap.push(fraction);
    copyMap.push.apply(copyMap, numeralWords.slice(8));
    return copyMap.length === 65 && copyMap.every(Boolean) ? copyMap : [];
  }

  function buildPage108Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg108_p001" && id !== "pg108_p034") return null;
    var section = content.querySelector('[data-section-id="pg108_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg108_p001") {
      var examples = section.querySelectorAll(".pg108-card:not(.pg108-exercise)");
      if (examples.length !== 3) return [];
      var exampleMap = [];
      [examples[0], examples[1]].forEach(function (example) {
        var items = example.querySelectorAll(".pg108-item");
        if (items.length !== 5) return;
        exampleMap.push.apply(exampleMap, ranges(example.querySelector(".pg108-banner")));
        repeat(exampleMap, items[0].querySelector(".pg108-picture"), 14);
        repeat(exampleMap, example.querySelector(".pg108-items"), 5);
        exampleMap.push.apply(exampleMap, ranges(items[1].querySelector(".pg108-label")));
        exampleMap.push.apply(exampleMap, ranges(items[2].querySelector(".pg108-label")));
        exampleMap.push.apply(exampleMap, ranges(items[3].querySelector(".pg108-label")));
        exampleMap.push(example.querySelector(".pg108-items"));
        exampleMap.push.apply(exampleMap, ranges(items[4].querySelector(".pg108-label")));
      });
      var thirdItems = examples[2].querySelectorAll(".pg108-item");
      if (thirdItems.length !== 5) return [];
      exampleMap.push.apply(exampleMap, ranges(examples[2].querySelector(".pg108-banner")));
      repeat(exampleMap, thirdItems[0].querySelector(".pg108-picture"), 10);
      [7, 8, 7, 8].forEach(function (clauseWords, itemIndex) {
        repeat(exampleMap, thirdItems[itemIndex + 1], clauseWords);
      });
      return exampleMap.length === 102 && exampleMap.every(Boolean) ? exampleMap : [];
    }

    var exercise = section.querySelector(".pg108-exercise");
    var instruction = exercise && exercise.querySelector(".pg108-instruction");
    var instructionWords = ranges(instruction);
    var fraction = instruction && instruction.querySelector(".pg108-fraction");
    var cells = exercise && exercise.querySelectorAll(".pg108-question .pg108-cell");
    if (!exercise || instructionWords.length !== 11 || !fraction || !cells || cells.length !== 4) return [];
    var exerciseMap = ranges(exercise.querySelector(".pg108-banner"));
    exerciseMap.push.apply(exerciseMap, instructionWords.slice(0, 5));
    exerciseMap.push(fraction);
    exerciseMap.push.apply(exerciseMap, instructionWords.slice(5));
    exerciseMap.push.apply(exerciseMap, ranges(cells[0]));
    repeat(exerciseMap, cells[1], 14);
    repeat(exerciseMap, cells[2], 6);
    repeat(exerciseMap, cells[3], 8);
    return exerciseMap.length === 43 && exerciseMap.every(Boolean) ? exerciseMap : [];
  }

  function buildPage109Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg109_p001" && id !== "pg109_p015") return null;
    var section = content.querySelector('[data-section-id="pg109_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg109_p001") {
      var table = section.querySelector(".pg109-table");
      var cells = table && table.children;
      if (!table || !cells || cells.length !== 16) return [];
      var continuationMap = [];
      repeat(continuationMap, table, 3);
      continuationMap.push.apply(continuationMap, ranges(cells[0]));
      repeat(continuationMap, cells[1], 14);
      repeat(continuationMap, cells[2], 5);

      continuationMap.push.apply(continuationMap, ranges(cells[4]));
      repeat(continuationMap, cells[5], 15);
      repeat(continuationMap, cells[6], 4);
      continuationMap.push.apply(continuationMap, ranges(cells[6]));
      repeat(continuationMap, cells[7], 4);

      continuationMap.push.apply(continuationMap, ranges(cells[8]));
      repeat(continuationMap, cells[9], 15);
      repeat(continuationMap, cells[10], 4);
      continuationMap.push.apply(continuationMap, ranges(cells[10]));
      repeat(continuationMap, cells[11], 4);

      continuationMap.push.apply(continuationMap, ranges(cells[12]));
      repeat(continuationMap, cells[13], 15);
      repeat(continuationMap, cells[14], 5);
      return continuationMap.length === 96 && continuationMap.every(Boolean) ? continuationMap : [];
    }

    var exercise = section.querySelector(".pg109-exercise");
    var instruction = exercise && exercise.querySelector(".pg109-instruction");
    var instructionWords = ranges(instruction);
    var fractions = instruction && instruction.querySelectorAll(".pg109-fraction");
    var items = exercise && exercise.querySelectorAll(".pg109-item");
    if (!exercise || instructionWords.length !== 10 || !fractions || fractions.length !== 2 || !items || items.length !== 6) return [];
    var exerciseMap = ranges(exercise.querySelector(".pg109-banner"));
    exerciseMap.push(instructionWords[0], instructionWords[1], fractions[0], instructionWords[2], instructionWords[3], instructionWords[4], fractions[1]);
    exerciseMap.push.apply(exerciseMap, instructionWords.slice(5));
    var descriptionLengths = [14, 14, 13, 15, 14, 14];
    items.forEach(function (item, itemIndex) {
      exerciseMap.push.apply(exerciseMap, ranges(item.querySelector(".pg109-item-no")));
      repeat(exerciseMap, item.querySelector(".pg109-shape"), descriptionLengths[itemIndex]);
    });
    return exerciseMap.length === 104 && exerciseMap.every(Boolean) ? exerciseMap : [];
  }

  function buildPage110Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg110_p001" && id !== "pg110_p004") return null;
    var section = content.querySelector('[data-section-id="pg110_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg110_p001") {
      var continuation = section.querySelector(".pg110-continuation");
      var zones = continuation && continuation.querySelectorAll(".pg110-question-zone");
      if (!continuation || !zones || zones.length !== 3) return [];
      var continuationMap = [];
      repeat(continuationMap, continuation, 3);
      repeat(continuationMap, zones[0], 9);
      repeat(continuationMap, zones[1], 16);
      repeat(continuationMap, zones[2], 16);
      return continuationMap.length === 44 && continuationMap.every(Boolean) ? continuationMap : [];
    }

    var heading = section.querySelector(".pg110-heading");
    var paragraphs = section.querySelectorAll(".pg110-copy");
    var orange = section.querySelector(".pg110-orange-card");
    if (!heading || paragraphs.length !== 3 || !orange) return [];
    var headingWords = ranges(heading);
    var firstParagraphWords = ranges(paragraphs[0]);
    var numeralWords = ranges(paragraphs[1]);
    var fractions = paragraphs[1].querySelectorAll(".pg110-fraction");
    var finalParagraphWords = ranges(paragraphs[2]);
    if (headingWords.length !== 8 || firstParagraphWords.length !== 48 || numeralWords.length !== 32 || fractions.length !== 2 || finalParagraphWords.length !== 12) return [];

    var articleMap = headingWords.concat(firstParagraphWords);
    articleMap.push.apply(articleMap, numeralWords.slice(0, 6));
    repeat(articleMap, fractions[0], 3);
    articleMap.push.apply(articleMap, numeralWords.slice(8, 21));
    repeat(articleMap, fractions[1], 3);
    articleMap.push.apply(articleMap, numeralWords.slice(23));
    articleMap.push.apply(articleMap, finalParagraphWords);
    repeat(articleMap, orange, 5);
    return articleMap.length === 107 && articleMap.every(Boolean) ? articleMap : [];
  }

  function buildPage111Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg111_p001" && id !== "pg111_p017") return null;
    var section = content.querySelector('[data-section-id="pg111_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg111_p001") {
      var caption = ranges(section.querySelector(".pg111-caption"));
      var removed = ranges(section.querySelector(".pg111-removed p"));
      var third = ranges(section.querySelector(".pg111-third > p:first-child"));
      var thirdLabel = ranges(section.querySelector(".pg111-third .pg111-fraction-label"));
      var remaining = ranges(section.querySelector(".pg111-remaining > p:first-child"));
      var remainingLabel = ranges(section.querySelector(".pg111-remaining .pg111-fraction-label"));
      var fractions = section.querySelectorAll(".pg111-fraction-label .pg111-frac");
      if (caption.length !== 8 || removed.length !== 7 || third.length !== 8 || thirdLabel.length !== 3 || remaining.length !== 10 || remainingLabel.length !== 3 || fractions.length !== 2) return [];
      var diagramMap = caption.concat(removed, third);
      diagramMap.push(thirdLabel[0]);
      repeat(diagramMap, fractions[0], 3);
      diagramMap.push.apply(diagramMap, remaining.slice(0, 8));
      diagramMap.push([remaining[8], remaining[9]]);
      diagramMap.push(remainingLabel[0]);
      repeat(diagramMap, fractions[1], 3);
      return diagramMap.length === 40 && diagramMap.every(Boolean) ? diagramMap : [];
    }

    var activity = section.querySelector(".pg111-activity");
    var activityParagraphs = activity && activity.querySelectorAll("p");
    if (!activity || !activityParagraphs || activityParagraphs.length !== 2) return [];
    var activityMap = ranges(activity.querySelector(".pg111-label"));
    activityMap.push.apply(activityMap, ranges(activityParagraphs[0]));
    activityMap.push.apply(activityMap, ranges(activityParagraphs[1]));
    return activityMap.length === 48 && activityMap.every(Boolean) ? activityMap : [];
  }

  function buildPage113Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg113_p001" && id !== "pg113_p007") return null;
    var section = content.querySelector('[data-section-id="pg113_sec001"]');
    if (!section) return [];
    ...
    function numberedItems(selector) {
      return Array.from(section.querySelectorAll(selector)).sort(function (left, right) {
        return parseInt(left.querySelector(".pg113-qno").textContent, 10) - parseInt(right.querySelector(".pg113-qno").textContent, 10);
      });
    }

    if (id === "pg113_p001") {
      var exampleOne = section.querySelector(".pg113-example-one");
      var oneItems = numberedItems(".pg113-one-item");
      if (!exampleOne || oneItems.length !== 4) return [];
      var oneMap = ranges(exampleOne.querySelector(".pg113-banner"));
      oneMap.push.apply(oneMap, ranges(exampleOne.querySelector(".pg113-instruction")));
      [16, 17, 17, 16].forEach(function (descriptionLength, itemIndex) {
        oneMap.push.apply(oneMap, ranges(oneItems[itemIndex].querySelector(".pg113-qno")));
        repeat(oneMap, oneItems[itemIndex].querySelector(".pg113-figure"), descriptionLength);
      });
      return oneMap.length === 80 && oneMap.every(Boolean) ? oneMap : [];
    }

    var exampleTwo = section.querySelector(".pg113-example-two");
    var twoItems = numberedItems(".pg113-two-item");
    if (!exampleTwo || twoItems.length !== 4) return [];
    var twoMap = ranges(exampleTwo.querySelector(".pg113-banner"));
    twoMap.push.apply(twoMap, ranges(exampleTwo.querySelector(".pg113-instruction")));
    [18, 16, 19, 16].forEach(function (descriptionLength, itemIndex) {
      twoMap.push.apply(twoMap, ranges(twoItems[itemIndex].querySelector(".pg113-qno")));
      repeat(twoMap, twoItems[itemIndex].querySelector(".pg113-figure"), descriptionLength - 1);
      twoMap.push.apply(twoMap, ranges(twoItems[itemIndex].querySelector(".pg113-caption")));
    });
    return twoMap.length === 82 && twoMap.every(Boolean) ? twoMap : [];
  }

  function buildPage114Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg114_p001" && id !== "pg114_p011") return null;
    var section = content.querySelector('[data-section-id="pg114_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg114_p001") {
      var exerciseFive = section.querySelector(".pg114-exercise-five");
      var items = Array.from(exerciseFive ? exerciseFive.querySelectorAll(".pg114-item") : []).sort(function (left, right) {
        return parseInt(left.querySelector(".pg114-qno").textContent, 10) - parseInt(right.querySelector(".pg114-qno").textContent, 10);
      });
      if (!exerciseFive || items.length !== 8) return [];
      var exerciseFiveMap = ranges(exerciseFive.querySelector(".pg114-label"));
      exerciseFiveMap.push.apply(exerciseFiveMap, ranges(exerciseFive.querySelector(".pg114-instruction")));
      [13, 13, 13, 14, 15, 13, 16, 13].forEach(function (descriptionLength, itemIndex) {
        exerciseFiveMap.push.apply(exerciseFiveMap, ranges(items[itemIndex].querySelector(".pg114-qno")));
        repeat(exerciseFiveMap, items[itemIndex].querySelector(".pg114-figure"), descriptionLength);
      });
      return exerciseFiveMap.length === 130 && exerciseFiveMap.every(Boolean) ? exerciseFiveMap : [];
    }

    var exerciseSix = section.querySelector(".pg114-exercise-six");
    var prompt = exerciseSix && exerciseSix.querySelector(".pg114-prompt");
    var answerGrid = exerciseSix && exerciseSix.querySelector(".pg114-answer-grid");
    if (!exerciseSix || !prompt || !answerGrid) return [];
    var exerciseSixMap = ranges(exerciseSix.querySelector(".pg114-label"));
    exerciseSixMap.push.apply(exerciseSixMap, ranges(exerciseSix.querySelector(".pg114-answer-heading")));
    exerciseSixMap.push.apply(exerciseSixMap, ranges(prompt.querySelector(".pg114-qno")));
    exerciseSixMap.push.apply(exerciseSixMap, ranges(prompt.querySelector("span:last-child")));
    repeat(exerciseSixMap, answerGrid, 13);
    return exerciseSixMap.length === 30 && exerciseSixMap.every(Boolean) ? exerciseSixMap : [];
  }

  function buildPage115Map(content, source) {
    var id = source.getAttribute("data-id");
    if (id !== "pg115_p001" && id !== "pg115_p004") return null;
    var section = content.querySelector('[data-section-id="pg115_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg115_p001") {
      var question = section.querySelector(".pg115-question");
      var promptWords = ranges(question && question.querySelector(".pg115-prompt"));
      var boxes = question && question.querySelector(".pg115-boxes");
      if (!question || promptWords.length !== 11 || !boxes) return [];
      var questionMap = ranges(question.querySelector(".pg115-question-number"));
      questionMap.push.apply(questionMap, promptWords.slice(0, 8));
      questionMap.push([promptWords[8], promptWords[9]]);
      questionMap.push(promptWords[10]);
      repeat(questionMap, boxes, 13);
      return questionMap.length === 24 && questionMap.every(Boolean) ? questionMap : [];
    }

    var exercise = section.querySelector(".pg115-exercise");
    var table = exercise && exercise.querySelector(".pg115-table");
    var cells = table && table.children;
    if (!exercise || !table || !cells || cells.length !== 24) return [];
    var exerciseMap = ranges(exercise.querySelector(".pg115-banner"));
    exerciseMap.push.apply(exerciseMap, ranges(exercise.querySelector(".pg115-instruction")));
    var headerRow = [cells[1], cells[2], cells[3]];
    repeat(exerciseMap, headerRow, 4);
    exerciseMap.push.apply(exerciseMap, ranges(cells[1]));
    exerciseMap.push.apply(exerciseMap, ranges(cells[2]));
    repeat(exerciseMap, cells[3], 8);

    var descriptionLengths = [10, 10, 10, 11, 11];
    descriptionLengths.forEach(function (descriptionLength, rowIndex) {
      var cellOffset = 4 + rowIndex * 4;
      exerciseMap.push.apply(exerciseMap, ranges(cells[cellOffset]));
      repeat(exerciseMap, cells[cellOffset + 1], descriptionLength);
      if (rowIndex === 0) {
        exerciseMap.push.apply(exerciseMap, ranges(cells[cellOffset + 2]));
        repeat(exerciseMap, cells[cellOffset + 3], 1);
        exerciseMap.push.apply(exerciseMap, ranges(cells[cellOffset + 3]));
      }
    });
    return exerciseMap.length === 91 && exerciseMap.every(Boolean) ? exerciseMap : [];
  }

  function buildPage117Map(content, source, narration) {
    var id = source.getAttribute("data-id");
    if (id !== "pg117_p001" && id !== "pg117_p020" && id !== "pg117_p033") return null;
    var section = content.querySelector('[data-section-id="pg117_sec001"]');
    var table = section && section.querySelector(".pg117-table");
    var rows = table ? Array.from(table.tBodies[0].rows) : [];
    if (!section || !table || rows.length !== 12) return [];

    function ranges(root) {
      return root ? collectRootTokens(root, content).map(function (token) { return token.range; }) : [];
    }
    function repeat(mapping, target, count) {
      for (var index = 0; index < count; index += 1) mapping.push(target);
    }
    function addRow(mapping, rowIndex, numeralCount, wordCount, wordIsBlank) {
      var cells = rows[rowIndex].cells;
      var number = ranges(cells[0]);
      if (number.length !== 1) return false;
      mapping.push(number[0]);
      repeat(mapping, cells[1], numeralCount);
      if (wordIsBlank) repeat(mapping, cells[2], wordCount);
      else {
        var wordRanges = ranges(cells[2]);
        if (wordRanges.length !== wordCount) return false;
        mapping.push.apply(mapping, wordRanges);
      }
      return true;
    }
    ...
    var valid = true;
    if (id === "pg117_p001") {
      mapping.push.apply(mapping, ranges(section.querySelector(".pg117-banner")));
      mapping.push.apply(mapping, ranges(section.querySelector(".pg117-instruction")));
      var headers = Array.from(table.tHead.rows[0].cells);
      repeat(mapping, [headers[1], headers[2]], 4);
      mapping.push.apply(mapping, ranges(headers[1]));
      mapping.push([headers[1], headers[2]]);
      mapping.push.apply(mapping, ranges(headers[2]));
      valid = addRow(mapping, 0, 3, 3, true) &&
        addRow(mapping, 1, 3, 1, false) &&
        addRow(mapping, 2, 3, 3, true) &&
        addRow(mapping, 3, 1, 3, true) &&
        addRow(mapping, 4, 3, 1, false) &&
        addRow(mapping, 5, 3, 2, false);
    } else if (id === "pg117_p020") {
      valid = addRow(mapping, 6, 3, 3, true) &&
        addRow(mapping, 7, 3, 1, false) &&
        addRow(mapping, 8, 3, 3, true) &&
        addRow(mapping, 9, 3, 1, false) &&
        addRow(mapping, 10, 3, 1, false);
    } else {
      valid = addRow(mapping, 11, 3, 1, false);
    }
    return valid && mapping.length === narration.length && mapping.every(Boolean) ? mapping : [];
  }

  function buildPage118Map(content, source, narration) {
    var id = source.getAttribute("data-id");
    if (id !== "pg118_p001" && id !== "pg118_p010") return null;
    var section = content.querySelector('[data-section-id="pg118_sec001"]');
    if (!section) return [];
    ...
    if (id === "pg118_p010") {
      var ictMap = ranges(section.querySelector(".pg118-ict h2"));
      ictMap.push.apply(ictMap, ranges(section.querySelector(".pg118-ict p")));
      return ictMap.length === narration.length && ictMap.every(Boolean) ? ictMap : [];
    }

    var mapping = ranges(section.querySelector(".pg118-banner"));
    mapping.push.apply(mapping, ranges(section.querySelector(".pg118-instruction")));
    var descriptionLengths = [14, 14, 6, 15, 15, 18];
    descriptionLengths.forEach(function (descriptionLength, itemIndex) {
      var printedNumber = String(itemIndex + 1);
      var cell = Array.from(section.querySelectorAll(".pg118-cell")).find(function (candidate) {
        var number = candidate.querySelector(".pg118-number");
        return number && normalizeWord(number.textContent) === printedNumber;
      });
      if (!cell) return;
      mapping.push.apply(mapping, ranges(cell.querySelector(".pg118-number")));
      repeat(mapping, cell.querySelector(".pg118-figure"), descriptionLength);
    });
    return mapping.length === narration.length && mapping.every(Boolean) ? mapping : [];
  }

  function buildPage119Map(content, source, narration) {
    if (source.getAttribute("data-id") !== "pg119_p001") return null;
    var section = content.querySelector('[data-section-id="pg119_sec001"]');
    if (!section) return [];
    ...
    var fractionParts = section.querySelectorAll(".fraction > span");
    var images = section.querySelectorAll(".screens img");
    var steps = section.querySelectorAll(".steps li");
    var expectedStepWords = [6, 9, 10, 6, 9];
    if (fractionParts.length !== 3 || images.length !== 2 || steps.length !== 5) return [];

    var mapping = ranges(section.querySelector(".banner"));
    mapping.push.apply(mapping, ranges(fractionParts[0]));
    mapping.push(fractionParts[1]);
    mapping.push.apply(mapping, ranges(fractionParts[2]));
    mapping.push.apply(mapping, ranges(section.querySelector(".solution h2")));

    var solutionWords = ranges(section.querySelector(".solution p"));
    if (solutionWords.length !== 18) return [];
    mapping.push.apply(mapping, solutionWords.slice(0, 10));
    mapping.push(solutionWords[9], solutionWords[9]);
    mapping.push.apply(mapping, solutionWords.slice(10, 13));
    mapping.push(solutionWords[12]);
    mapping.push(solutionWords[13], solutionWords[13], solutionWords[14]);
    mapping.push.apply(mapping, solutionWords.slice(15));

    repeat(mapping, [images[0], images[1]], 11);
    repeat(mapping, images[0], 5);
    repeat(mapping, images[1], 9);
    mapping.push.apply(mapping, ranges(section.querySelector(".steps h2")));

    for (var stepIndex = 0; stepIndex < steps.length; stepIndex += 1) {
      var number = ranges(steps[stepIndex].querySelector(".step-number"));
      var words = ranges(steps[stepIndex].querySelector(".step-text"));
      if (number.length !== 1 || words.length !== expectedStepWords[stepIndex]) return [];
      mapping.push(number[0]);
      mapping.push.apply(mapping, words);
    }
    return mapping.length === narration.length && mapping.every(Boolean) ? mapping : [];
  }

  function buildPage121Map(content, source, narration) {
    if (source.getAttribute("data-id") !== "pg121_p001") return null;
    var section = content.querySelector('[data-section-id="pg121_sec001"]');
    var rows = section ? Array.from(section.querySelectorAll(".pg121-position-row")) : [];
    if (!section || rows.length !== 4) return [];

    function ranges(root) {
      return root ? collectRootTokens(root, content).map(function (token) { return token.range; }) : [];
    }

    // "Exercise 1 continues" is narration-only. Leave those three tokens
    // unmapped so they do not paint a highlight over the entire page.
    var mapping = [null, null, null];
    rows.forEach(function (row) {
      var blank = row.querySelector(".pg121-blank");
      var words = ranges(row);
      if (!blank || words.length < 2) return;
      mapping.push.apply(mapping, words.slice(0, 4));
      mapping.push(blank);
      mapping.push.apply(mapping, words.slice(4));
    });
    return mapping.length === narration.length && mapping.slice(3).every(Boolean) ? mapping : [];
  }

  function buildPage131ImageMap(content, source, narration) {
    var id = source.getAttribute("data-id");
    if (id !== "pg131_im001" && id !== "pg131_im002") return null;
    var section = content.querySelector('[data-section-id="pg131_sec001"]');
    var image = section && section.querySelector(id === "pg131_im001" ? ".pg131-small-bucket" : ".pg131-cup");
    return image ? new Array(narration.length).fill(image) : [];
  }

  function buildPage134SecondColumnImageMap(content, source, narration) {
    var id = source.getAttribute("data-id");
    var selectors = {
      pg134_im002: 'img[src="images/pg134_im002.png"]',
      pg134_im004: 'img[src="images/pg134_im004.png"]',
      pg134_im006: 'img[src="images/pg134_im006.png"]'
    };
    if (!selectors[id]) return null;
    var section = content.querySelector('[data-section-id="pg134_sec001"]');
    var image = section && section.querySelector(selectors[id]);
    return image ? new Array(narration.length).fill(image) : [];
  }

  function buildPage135SecondColumnImageMap(content, source, narration) {
    var id = source.getAttribute("data-id");
    var selectors = {
      pg135_im002: 'img[src="images/pg135_im002.png"]',
      pg135_im004: 'img[src="images/pg135_im004.png"]',
      pg135_im006: 'img[src="images/pg135_im006.png"]',
      pg135_im008: 'img[src="images/pg135_im008.png"]',
      pg135_im010: 'img[src="images/pg135_im010.png"]'
    };
    if (!selectors[id]) return null;
    var section = content.querySelector('[data-section-id="pg135_sec001"]');
    var image = section && section.querySelector(selectors[id]);
    return image ? new Array(narration.length).fill(image) : [];
  }

  function buildPage135Item4Map(content, source, narration) {
    if (source.getAttribute("data-id") !== "pg135_p001") return null;
    var section = content.querySelector('[data-section-id="pg135_sec001"]');
    var row = section && section.querySelector(".pg135-row:nth-child(1)");
    if (!row) return [];
    var mapping = new Array(narration.length).fill(null);
    var targets = [row.querySelector(".pg135-number")].concat(Array.from(row.querySelectorAll(".pg135-label")));
    var visible = [];
    targets.filter(Boolean).forEach(function (target) {
      visible.push.apply(visible, collectRootTokens(target, content));
    });
    var itemStart = narration.indexOf("4");
    visible.forEach(function (token, index) {
      if (itemStart + index < mapping.length && narration[itemStart + index] === token.normalized) {
        mapping[itemStart + index] = token.range;
      }
    });
    return mapping;
  }

  function buildPage136Exercise2HeadingMap(content, source, narration) {
    if (source.getAttribute("data-id") !== "pg136_p012") return null;
    var section = content.querySelector('[data-section-id="pg136_sec001"]');
    var heading = section && section.querySelector(".pg136-card-two .pg136-label");
    if (!heading) return [];
    var mapping = alignTokens(narration, collectVisibleTokens(content));
    var headingTokens = collectRootTokens(heading, content);
    headingTokens.forEach(function (token, index) {
      if (index < narration.length && narration[index] === token.normalized) mapping[index] = token.range;
    });
    return mapping;
  }

  function buildPage137Map(content, source, narration) {
    var id = source.getAttribute("data-id");
    if (id !== "pg137_p001" && id !== "pg137_p007") return null;
    var section = content.querySelector('[data-section-id="pg137_sec001"]');
    if (!section) return [];
    var mapping = new Array(narration.length).fill(null);

    if (id === "pg137_p001") {
      var cursor = 3;
      section.querySelectorAll(".pg137-questions li").forEach(function (question) {
        [question.querySelector(".pg137-question-number"), question.querySelector(".pg137-question-text")].forEach(function (root) {
          collectRootTokens(root, content).forEach(function (token) {
            if (cursor < narration.length && narration[cursor] === token.normalized) {
              mapping[cursor] = token.range;
              cursor += 1;
            }
          });
    ...
    [section.querySelector(".pg137-label"), section.querySelector(".pg137-title"), section.querySelector(".pg137-instruction")].forEach(function (root) {
      collectRootTokens(root, content).forEach(function (token) {
        if (activityCursor < narration.length && narration[activityCursor] === token.normalized) {
          mapping[activityCursor] = token.range;
          activityCursor += 1;
        }
      });
    });
    var picture = section.querySelector(".pg137-picture");
    while (activityCursor < narration.length) {
      mapping[activityCursor] = picture;
      activityCursor += 1;
    }
    return mapping;
  }

  function buildPage138QuestionsHeadingMap(content, source, narration) {
    if (source.getAttribute("data-id") !== "pg138_p020") return null;
    var section = content.querySelector('[data-section-id="pg138_sec001"]');
    var heading = section && section.querySelector(".pg138-questions .pg138-banner");
    if (!heading) return [];
    var mapping = alignTokens(narration, collectVisibleTokens(content));
    var headingToken = collectRootTokens(heading, content)[0];
    if (headingToken && narration[0] === headingToken.normalized) mapping[0] = headingToken.range;
    return mapping;
  }

  function buildPage139Map(content, source, narration) {
    var id = source.getAttribute("data-id");
    if (id !== "pg139_p001" && id !== "pg139_p010") return null;
    var section = content.querySelector('[data-section-id="pg139_sec001"]');
    if (!section) return [];
    var mapping = new Array(narration.length).fill(null);

    function mapRoot(root, cursor) {
      var tokens = collectRootTokens(root, content);
      for (var index = 0; index < tokens.length; index += 1) {
        var token = tokens[index];
        if (cursor < narration.length && narration[cursor] === token.normalized) {
          mapping[cursor] = token.range;
          cursor += 1;
        } else if (cursor < narration.length && index + 1 < tokens.length && narration[cursor] === token.normalized + tokens[index + 1].normalized) {
          var combined = document.createRange();
          combined.setStart(token.range.startContainer, token.range.startOffset);
          combined.setEnd(tokens[index + 1].range.endContainer, tokens[index + 1].range.endOffset);
          mapping[cursor] = combined;
          cursor += 1;
          index += 1;
        }
      }
    ...
    if (id === "pg139_p001") {
      var continuationCursor = 3;
      section.querySelectorAll(".pg139-continuation li").forEach(function (item) {
        continuationCursor = mapRoot(item.querySelector(".pg139-number"), continuationCursor);
        continuationCursor = mapRoot(item.querySelector("span:last-child"), continuationCursor);
      });
      return mapping;
    }

    var exerciseCursor = 0;
    exerciseCursor = mapRoot(section.querySelector(".pg139-banner"), exerciseCursor);
    exerciseCursor = mapRoot(section.querySelector(".pg139-intro"), exerciseCursor);
    var shapes = section.querySelector(".pg139-shapes");
    while (exerciseCursor <= 16) mapping[exerciseCursor++] = shapes;
    var shapeImages = section.querySelectorAll(".pg139-shapes img");
    while (exerciseCursor <= 30) mapping[exerciseCursor++] = shapeImages[0];
    while (exerciseCursor <= 44) mapping[exerciseCursor++] = shapeImages[1];
    while (exerciseCursor <= 48) mapping[exerciseCursor++] = shapeImages[2];
    exerciseCursor = mapRoot(section.querySelector(".pg139-question-title"), exerciseCursor);
    section.querySelectorAll(".pg139-questions li").forEach(function (item) {
      exerciseCursor = mapRoot(item.querySelector(".pg139-number"), exerciseCursor);
      exerciseCursor = mapRoot(item.querySelector("span:last-child"), exerciseCursor);
    });
    return mapping;
  }
